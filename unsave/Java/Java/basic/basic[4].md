...
- $\textcolor{#2a6e3f}{【09】}$ [javabean](#1.9)
- $\textcolor{#2a6e3f}{【10】}$ [package](#1.10)
...
- 如果没有定义构造器，编译器默认提供空的构造器（初始化时，变量均采用默认值）
...
public Person(String name, int age,int floor) {
    this(name,age);
    this.floor = floor;
}
```
...
### 9、javabean<a id="1.9">💛</a>

1. 类是公共的 
2. 有一个无参的公共的构造器 
3. 有属性，且有对应的`get`、`set`方法

### 10、package<a id="1.10">💛</a>
...
- 个人开发
  ...
  - indi：多人完成，版权属于发起者（individual）

  ```sql
  # 包名为indi.发起者名.项目名.模块名
  ```

  - pers ：独自完成，公开，版权主要属于个人。（personal）

  ```sql
  # 包名为pers.个人名.项目名.模块名
  ```

  - priv ： 独自完成，非公开，版权属于个人。（private）
